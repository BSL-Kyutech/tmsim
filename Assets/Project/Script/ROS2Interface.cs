...
public class ROS2Interface : MonoBehaviour
{
    ...
    private IPublisher<std_msgs.msg.Float64>[] inclinationPub;
    ...
    void Start()
    {
        ...
        inclinationPub = new IPublisher<std_msgs.msg.Float64>[numStruts];
        ...
        if (ros2Unity.Ok())
        {
            ...
            for (int i = 0; i < numStruts; i++)
            {
                ...
                inclinationPub[i] = ros2Node.CreatePublisher<std_msgs.msg.Float64>(
                    $"/{dev.name}/strut{i}/inclination");
            }
    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < dev.numLayer * dev.numPrism; i++)
        {
            ...
            // Compute inclination angle , world frame  to body frame
            Vector3 strutUpWorld = strutTransform.up;
            double inclinationAngle = Mathf.Acos(Vector3.Dot(strutUpWorld, Vector3.up) / (strutUpWorld.magnitude * Vector3.up.magnitude));
            ...
            var inclinationMsg = new std_msgs.msg.Float64();
            inclinationMsg.Data = inclinationAngle;
            inclinationPub[i].Publish(inclinationMsg);
        }
    }
